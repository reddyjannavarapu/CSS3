...
$(document).ready(function () {
    ...
    $("#btnCss1Review").click(function () {
        try {
            BindCss1Review($GlobalDataWOTYPE.WOID, $GlobalDataWOTYPE.Type);

            //$('.ace-scroll').ace_scroll({ size: 400, styleClass: 'scroll-visible' });
            $('.ace-scroll').ace_scroll({ size: 400 });
    ...
    function BindCss1Review(WOID, Wotype) {
        try {
            $.ajax({
                type: "POST",
                url: "/Css1Review/Css1ReviewDetails",
                ...
                success: function (msg) {
                    try {
                        if (msg == '0') {
                            ...
                            BindCss1ReviewDynamicTables(msg);
                        }
    ...
    function BindCss1ReviewDynamicTables(msg) {
        try {

            var ddlID = '1';
            $('#OutputReview_' + ddlID).html('');
            ...
            $.each(result, function (index, data) {
                ...
                if (data[0] != undefined) {
                    ...
                    $('#OutputReview_' + ddlID).append(Title);
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        table.append(tr);
                        $('#OutputReview_' + ddlID).append(table);
                    }
                }
            });

            if (IsEmpty == '0') {
                $('#OutputReview_' + ddlID).html('<h5 class="red medium"> No data found.</h5>');
                return false;
            }
